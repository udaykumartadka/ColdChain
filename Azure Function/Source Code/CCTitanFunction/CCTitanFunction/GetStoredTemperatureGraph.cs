...
{
    public static class GetStoredTemperatureGraph
    {
        [FunctionName("GetStoredTemperatureGraph")]
        public static async Task<HttpResponseMessage> Run([HttpTrigger(AuthorizationLevel.Anonymous, "get", "post", Route = null)]HttpRequestMessage req, TraceWriter log)
        {
            ...
            List<StoredTemperatureGraph> objTempList = new List<StoredTemperatureGraph>();

            string cosmosTelemeterySelect = "SELECT TOP 100 c.temperature,f.current_system_time FROM f JOIN c IN  f.sensor_Values WHERE c.sensorID = '" + BeaconId + "' and (f.message_type='Stored_Telemetry' or f.message_type='Stored_Alert')   and c.temperature !=0 order by f.current_system_time desc"; 
            IQueryable<StoredTemperatureGraph> cosTelemetry = client.CreateDocumentQuery<StoredTemperatureGraph>(collectionlinK, cosmosTelemeterySelect);
    ...
    public class StoredTemperatureGraph
    {
