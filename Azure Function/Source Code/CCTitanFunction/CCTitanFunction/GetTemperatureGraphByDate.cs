...
{
    public static class GetTemperatureGraphByDate
    {
        [FunctionName("GetTemperatureGraphByDate")]
        public static async Task<HttpResponseMessage> Run([HttpTrigger(AuthorizationLevel.Anonymous, "get", "post", Route = null)]HttpRequestMessage req, TraceWriter log)
        {
            ...
            List<TempGraph> objTempList = new List<TempGraph>();
            ...
            string cosmosTelemeterySelect = "SELECT c.temperature,f.current_system_time FROM f JOIN c IN  f.sensor_Values WHERE c.sensorID = '" + beaconId + "'"+
                    " and f.current_system_time >= '" + fromDate + "' and f.current_system_time < '" + toDate + "'  and c.temperature !=0 order by f.current_system_time asc";
            IQueryable<TempGraph> cosTelemetry = client.CreateDocumentQuery<TempGraph>(collectionlinK, cosmosTelemeterySelect);
